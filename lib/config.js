...
export const addProxy = (name, port, target) => {
  const proxies = getProxies();
  proxies[name] = { port, target, registeredAt: new Date().toISOString() };
  setConfig('proxies', proxies);
};

export const updateProxy = (name, options = {}) => {
  const proxies = getProxies();
  if (proxies[name]) {
    if (options.port) proxies[name].port = options.port;
    if (options.target) proxies[name].target = options.target;
    proxies[name].updatedAt = new Date().toISOString();
    setConfig('proxies', proxies);
  }
};

export const removeProxy = (name) => {
  const proxies = getProxies();
  if (proxies[name]) {
    delete proxies[name];
    setConfig('proxies', proxies);
